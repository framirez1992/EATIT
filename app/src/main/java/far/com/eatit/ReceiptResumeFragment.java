...
public class ReceiptResumeFragment extends Fragment {
    ...
    public void refreshList(){
        ...
        Receipts receipt = SalesController.getInstance(activity).getReceiptByCodeAreadetail(/*codeAreaDetail*/0);
        if(receipt != null){
        ...
        if(activity instanceof MainReceipt){
            PaymentDialog p = ((MainReceipt)activity).paymentDialog;
            if(p != null && p.getDialog()!= null && p.getDialog().isShowing()){
                p.refreshAmount();
            }
        }
