...
public class OrderController {
    ...
    public static String IDORDER="idOrder", IDUSER="idUser",IDAREADETAIL="idAreaDetail",SUBTOTAL="subTotal",
            DISCOUNT="discount",TAXES="taxes",TOTAL="total", STATUS = "status",NOTES = "notes",
            CANCELREASON = "cancelReason", ORDERINFOJSON="orderInfoJson",
    ...
    String[]columns = new String[]{IDORDER,IDUSER,IDAREADETAIL, SUBTOTAL, DISCOUNT, TAXES, TOTAL, STATUS,NOTES, CANCELREASON, ORDERINFOJSON, CREATEDATE, CREATEUSER,UPDATEDATE, UPDATEUSER, DELETEDATE, DELETEUSER};

    public static final String TABLE_NAME_DETAIL = "ORDERDETAILS";
    public static String IDORDERDETAIL = "idOrderDetail",DETAIL_IDORDER = "idOrder", DETAIL_IDPRODUCT = "idProduct",DETAIL_IDPRODUCTMEASURE="idProductMeasure",
    ...
    String[]columnsDetails = new String[]{IDORDERDETAIL,DETAIL_IDORDER, DETAIL_IDPRODUCT,DETAIL_IDPRODUCTMEASURE,DETAIL_QUANTITY,DETAIL_PRICE,DETAIL_DISCOUNT,DETAIL_TAX,DETAIL_MANUALPRICE,DETAIL_POSITION,DETAIL_CREATEDATE, DETAIL_CREATEUSER, DETAIL_UPDATEDATE, DETAIL_UPDATEUSER,DETAIL_DELETEDATE,DETAIL_DELETEUSER};
    ...
    private OrderController(Context c){
        this.context = c;
        this.db = DB.getInstance(c);
    }
    public static OrderController getInstance(Context c){
        if(instance == null){
            instance = new OrderController(c);
        }
        return instance;
    }
    ...
    public static String getQueryCreateDetail(){
        String QUERY_CREATE_DETAIL = "CREATE TABLE "+TABLE_NAME_DETAIL+" ("
                +IDORDERDETAIL+" INTEGER, "
                +DETAIL_IDORDER+" INTEGER, "
    ...
    public void insertOrUpdate(Order obj){
        String sql ="insert or replace into "+TABLE_NAME+" ("+IDORDER+", "+IDUSER+", "+IDAREADETAIL+","+SUBTOTAL+","+DISCOUNT+","+TAXES+","+TOTAL+","+STATUS+","+NOTES+","+CANCELREASON+","+ORDERINFOJSON +", "+CREATEDATE+", "+CREATEUSER+","+UPDATEDATE+", "+UPDATEUSER+", "+DELETEDATE+", "+DELETEUSER+") values " +
                "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        db.getWritableDatabase().execSQL(sql,new String[]{obj.getId()+"", obj.getIduser()+"", obj.getIdAreaDetail(),obj.getSubTotal()+"",obj.getDiscount()+"",obj.getTaxes()+"",obj.getTotal()+"",obj.getStatus()+"",obj.getNotes(),obj.getCancelReason(),obj.getOrderInfoJson(), obj.getCreateDate(), obj.getCreateUser(),  obj.getUpdateDate(),obj.getUpdateUser(),  obj.getDeleteDate(), obj.getDeleteUser() });
    }

    public void insertOrUpdate(OrderDetail obj){
        String sql ="insert or replace into "+TABLE_NAME_DETAIL+" ("+IDORDERDETAIL+", "+DETAIL_IDORDER+", "+DETAIL_IDPRODUCT+", "+DETAIL_IDPRODUCTMEASURE+","+DETAIL_QUANTITY+","+DETAIL_PRICE+","+DETAIL_DISCOUNT+","+DETAIL_TAX+","+DETAIL_MANUALPRICE+","+DETAIL_POSITION+", "+DETAIL_CREATEDATE+", "+DETAIL_CREATEUSER+","+DETAIL_UPDATEDATE+", "+DETAIL_UPDATEUSER+", "+DETAIL_DELETEDATE+", "+DETAIL_DELETEUSER+") values " +
                "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public long insert(Sale obj){
        ...
        cv.put(ORDERINFOJSON, obj.getSaleInfoJson());
    ...
    public long update(Sale obj){
        ...
        String table = TABLE_NAME;
        long result = DB.getInstance(context).getWritableDatabase().update(table,cv,IDORDER.concat(" = ? "), new String[]{obj.getId()+""} );
        return result;
    }

    public long delete(Sale obj){
        String table =TABLE_NAME;
        long result = DB.getInstance(context).getWritableDatabase().delete(table,IDORDER.concat(" = ? "), new String[]{obj.getId()+""});
        return result;
    }
    ...
    public long insert_Detail(SaleDetail obj){
        ContentValues cv = new ContentValues();
        cv.put(IDORDERDETAIL,obj.getId());
        cv.put(DETAIL_IDORDER, obj.getIdsale());
    ...
    public long update_Detail(SaleDetail obj){
        ...
        long result = DB.getInstance(context).getWritableDatabase().update(TABLE_NAME_DETAIL,cv,IDORDERDETAIL.concat("= ?"), new String[] {obj.getId()+""});
        return result;
    }


    public ArrayList<Sale> getOrders(String where, String[] args){
    ...
    public ArrayList<SalesDetails> getOrderDetailsByIdOrder(int idOrder){
        String where = DETAIL_IDORDER+" = ?";
        return getOrderDetails(where, new String[]{idOrder+""});
    }
    public ArrayList<SalesDetails> getOrderDetails(String where, String[] args){
    ...
    public Sale getOrderById(int id){
        String table = TABLE_NAME;
        Sale s = null;
        String where = IDORDER+" = ?";
    ...
    public ArrayList<OrderModel> getOrderModels(String where){
        ...
        String sql = "Select DISTINCT "+IDORDER+","+STATUS+", "+NOTES+", "+DISCOUNT+", "+TOTAL+", " +CREATEDATE+", "+UPDATEDATE+" "+
                "FROM "+TABLE_NAME+" " +
        ...
        while(c.moveToNext()){
            ...
            int idSale = c.getInt(c.getColumnIndex(IDORDER));
    ...
    public ArrayList<WorkedOrdersRowModel> getWorkedOrderModels(String where){
        ...
        try {
            String sql = "Select DISTINCT s." + IDORDER + " as CODE, ifnull(ifnull(s." + UPDATEDATE + ", s." + CREATEDATE + "), 'NONE') as FECHA,  " +
                    "ad." + AreasDetailController.IDAREADETAIL + " as ADCODE, ad." + AreasDetailController.DESCRIPTION + " as ADDESCRIPTION, s." + STATUS + " as STATUS, " +
                    "s."+TOTAL+" as TOTAL,u."+UsersController.USERNAME+" as USERNAME  " +
                    "FROM " + TABLE_NAME + " s  " +
                    "INNER JOIN " + AreasDetailController.TABLE_NAME + " ad on ad." + AreasDetailController.IDAREADETAIL + " = s." + IDAREADETAIL + " " +
                    ((where == null) ? "" : "WHERE " + where + " ")
    ...
    public ArrayList<OrderDetailModel> getOrderDetailModels(int idSale){
        ...
        try {
            String where = "sd."+DETAIL_IDORDER + " = '" + idSale +"'";

            String sql = "Select sd."+IDORDERDETAIL+" as CODE,sd."+DETAIL_IDORDER+" as CODESALES,p."+ProductsController.CODE+" AS CODEPRODUCT, " +
    ...
    public ArrayList<SelectableOrderRowModel> getSelectableOrderModelsByAreaDetail(int tableId){
        ...
        String sql = "Select DISTINCT s." + IDORDER + " as CODE, ifnull(ifnull(s." + UPDATEDATE  + ", s." + CREATEDATE + "), 'NONE') as FECHA,  " +
                ...
                "WHERE ad."+AreasDetailController.IDAREADETAIL+" = ? AND s."+STATUS+" IN(?, ?, ?) "+
                "ORDER BY ad." + AreasDetailController.DESCRIPTION;
    ...
    public ArrayList<OrderReceiptModel> getOrderReceipt(int idTable){
        ...
        String sql = "SELECT  ad."+AreasDetailController.IDAREADETAIL+" AS CODEMESA, ad."+AreasDetailController.DESCRIPTION+" AS DESCRIPTIONMESA, " +
                "sum(s."+OrderController.TOTAL+") AS SALESTOTAL " +
                "FROM "+OrderController.TABLE_NAME+" s " +
                "INNER JOIN "+AreasDetailController.TABLE_NAME+" ad ON s."+IDAREADETAIL+" = ad."+AreasDetailController.IDAREADETAIL+" " +
                "WHERE s."+OrderController.STATUS+" = ? AND s."+OrderController.IDAREADETAIL+" = ? " +
                "GROUP BY a."+AreasController.CODE+", ad."+AreasDetailController.IDAREADETAIL;
    ...
    public ArrayList<ReceiptResumeModel> getOrderReceiptResume(String codeAreaDetail){
        ...
        String sql = "SELECT sd."+OrderController.DETAIL_IDPRODUCT+" AS CODEPRODUCT, p."+ProductsController.DESCRIPTION+" AS  PRODUCTDESCRIPTION, " +
                "sd."+OrderController.DETAIL_IDPRODUCTMEASURE +" AS CODEMEASURE, mu."+MeasureUnitsController.DESCRIPTION+" AS MEASUREDESCRIPTION, "+
                "SUM(sd."+OrderController.DETAIL_QUANTITY+") AS QUANTITY, SUM(sd."+OrderController.DETAIL_PRICE+" * "+OrderController.DETAIL_QUANTITY+" ) AS SALESTOTAL "+
                "FROM "+OrderController.TABLE_NAME+" s " +
                "INNER JOIN "+OrderController.TABLE_NAME_DETAIL+" sd on s."+OrderController.IDORDER +" = sd."+OrderController.DETAIL_IDORDER+" "+
                "INNER JOIN "+ProductsController.TABLE_NAME+" p on p."+ProductsController.CODE+" = sd."+OrderController.DETAIL_IDPRODUCT+" "+
                "LEFT JOIN "+MeasureUnitsController.TABLE_NAME+" mu on mu."+MeasureUnitsController.CODE+" = sd."+OrderController.DETAIL_IDPRODUCTMEASURE+" "+
                "INNER JOIN "+AreasDetailController.TABLE_NAME+" ad ON s."+IDAREADETAIL+" = ad."+AreasDetailController.IDAREADETAIL+" " +
                "WHERE s."+OrderController.STATUS+" = ? AND s."+OrderController.IDAREADETAIL+" = ? " +
                "GROUP BY sd."+OrderController.DETAIL_IDPRODUCT+",sd."+OrderController.DETAIL_IDPRODUCTMEASURE+" " +
    ...
    public ArrayList<Sales> getDeliveredOrdersByCodeAreadetail(int idTable){
        ...
        String where = OrderController.STATUS+" = ? AND "+OrderController.IDAREADETAIL+" = ? ";
    ...
    public Receipts getReceiptByCodeAreadetail(int idTable){
        ...
        String where = "s."+OrderController.STATUS+" = ? AND s."+OrderController.IDAREADETAIL+" = ? ";
        String sql = "SELECT SUM(sd."+OrderController.DETAIL_PRICE+" * sd."+OrderController.DETAIL_QUANTITY+" ) AS TOTAL " +
                "FROM "+OrderController.TABLE_NAME+" s " +
                "INNER JOIN "+OrderController.TABLE_NAME_DETAIL+" sd on s."+OrderController.IDORDER+" = sd."+OrderController.DETAIL_IDORDER+" "+
                " WHERE "+where;
